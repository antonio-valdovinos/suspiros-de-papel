...
export default function Services() {
  ...
  return (
    <section
      ref={ref}
      id="services"
      className="scroll-reveal py-40 px-8 relative overflow-hidden line-art-bg"
    >
      {/* Blob decorativo */}
      ...
      <div className="max-w-7xl mx-auto text-center mb-24 relative">
        <h2 className="text-5xl md:text-7xl font-headline font-bold mb-6 text-on-surface">
          ¿Tienes un proyecto{" "}
          <br />
      ...
      <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        {services.map((service) => {
          ...
          return (
            <div
              key={service.title}
              className={`group relative aspect-[3/4] rounded-[2.5rem] overflow-hidden shadow-lg hover:shadow-2xl transition-all duration-500 hover:-translate-y-2 ${s.card} ${s.offset}`}
            >
              {/* Icono */}
              <div className={`absolute top-10 left-10 w-16 h-16 rounded-full flex items-center justify-center group-hover:scale-110 transition-transform ${s.iconBg} ${s.iconText}`}>
                {service.icon}
              </div>
